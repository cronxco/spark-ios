...
- `xcodebuild build -workspace Spark.xcworkspace -scheme SparkApp -destination 'platform=iOS Simulator,name=iPhone 18 Pro Max,OS=27.0' -configuration Debug` builds the main app.
- `cd Packages/SparkKit && swift test` runs the fastest package-level unit tests.
- `xcodebuild -workspace Spark.xcworkspace -scheme SparkApp -destination 'platform=iOS Simulator,name=iPhone 18 Pro Max,OS=27.0' -skipPackagePluginValidation -skipMacroValidation test` runs the app test scheme.
